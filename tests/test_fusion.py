...
# Skip the entire module if PyTorch isn't available. The real project uses
# torch, but it's an optional dependency in these kata tests.
    ...
    return {
        "fusion": {
            ...
            "moe": {
                "num_experts": 3,
                "expert_hidden": 128,
                "gating": {"source": "concat", "hidden": 64},
            },
        }
    }
...
def test_factory_and_forward_all():
    ...
    for t in variants:
        ...
        y, extras = fuser(
            x1,
            x2,
            molecule=torch.ones(B),
            seam=torch.zeros(B),
            wavepos=torch.linspace(0, 1, B),
            snr=torch.full((B,), 50.0),
        )
